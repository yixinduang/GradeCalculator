...
const FinalTable = () => {
    ...
    React.useEffect(() => {
        ...
        for (let j = 0; j < gradingComp.length; j++) {
            ...
            for (let i = 1; i <= g.number; i++) {
                ...
                const component = { cname: g.name + i, cpercenatge: Number(g.percentage), score: 0 }
                componentList.push(component);
                console.log("ive runed")
    ...
    function handleChange(event) {
        ...
        const gradelist = componentList.map(c =>
            c.score * c.cpercenatge / 100
        ...
        for (let i = 1; i <= item.number; i++) {
            ...
            sectionList.push(
                <tr id="body">
                    ...
                    <th><input type="number" name={n + i} onChange={handleChange} />% </th>
    ...
    return (
        <>
            <Nav np={"Now its time to calucate your grade! enter how much you have earned out of the total below.Both fraction and percentage is acceptable (exp.49/50 or 98%)."}/>
            <table>
                <>{sectionContent}</>
